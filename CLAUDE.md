...
- `public/sw.js` — service worker with stale-while-revalidate strategy. Cache name is `flota-v11`. **When deploying breaking JS changes, bump the cache name** so old clients clear their cache on next activation.
